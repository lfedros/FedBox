...
if isequal(options.targetFrame, 'auto')
    ...
    yy = linspace(1, sz(1), 4);
    xx = linspace(1, sz(2), 4);
    iRect = 0;
    for iY = 1:length(yy)-1
        for iX = 1:length(xx)-1
            iRect = iRect+1;
            posBank(iRect).ymin = max(1, floor(yy(iY)));
            posBank(iRect).ymax = min(sz(2), ceil(yy(iY+1)));
            posBank(iRect).xmin = max(1, floor(xx(iX)));
            posBank(iRect).xmax = min(sz(2), ceil(xx(iX+1)));
        end
    end
...
for iRect = 1:nRects
    fclose(fid(iRect));
    info.targetFrame = targetBank{iRect};
    info.validX = validX{iRect};
    info.validY = validY{iRect};
    info.dx = allDx(:, iRect);
    info.dy = allDy(:, iRect);
...
% close parallel pookl, if it was opened here
