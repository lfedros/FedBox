...
% catch
    ...
    info.folder2p=fullfile('\\zserver2\Data\2Photon\imaging\', info.subject, info.expDate, num2str(info.exp));
    info.basename2p=sprintf('%s_%d_%s_2P', info.expDate, info.exp, info.subject);
    info.folderTL=fullfile('\\zserver\Data\expInfo\', info.subject, info.expDate, num2str(info.exp));
    info.basenameTL=sprintf('%s_%d_%s_Timeline', info.expDate, info.exp, info.subject);
% end

% after recent paths change we should do this: (MK 2014-06-30)
% catch
    % this is a hack while the previous code is buggy
    % however, the paths are hard-coded, so be careful
    info.folder2p=fullfile('\\zserver2\Data\2P\', info.subject, info.expDate, num2str(info.exp));
    info.basename2p=sprintf('%s_%d_%s_2P', info.expDate, info.exp, info.subject);
    info.folderTL=fullfile('\\zserver\Data\expInfo\', info.subject, info.expDate, num2str(info.exp));
    info.basenameTL=sprintf('%s_%d_%s_Timeline', info.expDate, info.exp, info.subject);
% end
