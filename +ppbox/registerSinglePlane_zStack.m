function info = registerSinglePlane_zStack(info, options)
...
for iCh = 1:info.nChannels
    nFr(iCh) = numel(info.chData(iCh).meanIntensity);
end
...
%     if indEnd <= nFr(iCh)% 170212 LFR: info.chData(iCh) might have different size for different channels
...
%     else 
        newEnd = min(nFr);
        info.chData(iCh).meanIntensity = info.chData(iCh).meanIntensity(indStart:newEnd);
        info.chData(iCh).tiffFrames = info.chData(iCh).tiffFrames(indStart:newEnd);      
        realSz{iCh} = sz;
        realSz{iCh}(3) = nFr(iCh);

%     end
end

indEnd = min(nFr);
nFrames = indEnd-sum(nFrames2Skip);
sz(3) = nFrames;
m.Format =  {'int16', sz, 'frames'};
...
for iChunk = 1:nChunks
    ...
    planeData = loadMovieFrames(filePath, frameStart, frameEnd, realSz{iCh}, prec);
